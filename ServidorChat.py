...
class ServidorChat:
    ...
    # Por defecto escuchamos en 0.0.0.0 para permitir conexiones desde otras PCs en la misma red.
    ...
    # -------------------------
    def enviar_linea(self, sock, texto):
        """Envía una línea completa terminada en \n.

        Nota: En TCP no existen "paquetes de mensaje"; pueden juntarse o partirse.
        Por eso enviamos y procesamos mensajes por líneas (\n) para evitar que se peguen.
        """
        try:
            sock.sendall((texto + "\n").encode('utf-8'))
        except Exception:
            # Si falla el envío, el hilo que llamó limpiará al cliente.
            pass
        ...
        try:
            while self.ejecutando:
                try:
                    ...
                    # Procesar mensajes por línea (delimitados por \n)
                    while "\n" in buffer:
                        ...
                        respuesta = self.procesar_mensaje_protocolo(linea, cliente_socket, direccion, nombre_cliente)
                    ...
                    # Compatibilidad: si el cliente manda sin \n, procesamos el chunk como un mensaje
                    # (Esto evita que se quede "atorado" si alguien usa telnet/netcat sin \n)
                    if "\n" not in buffer and "|" in buffer and len(buffer) > 2048:
                        linea = buffer
                        buffer = ""
                        respuesta = self.procesar_mensaje_protocolo(linea, cliente_socket, direccion, nombre_cliente)
                        if respuesta and 'nombre_registrado' in respuesta:
    ...
    def procesar_mensaje_protocolo(self, mensaje, cliente_socket, direccion, nombre_actual):
        ...
        try:
            ...
            if comando == "CONNECT": # Manejar conexión inicial
                ...
                self.enviar_linea(cliente_socket, "PONG")
                return None
            
            else: # Comando no reconocido
                self.enviar_linea(cliente_socket, "ERROR|Comando no reconocido")
                return None
                
        except Exception as e: # Manejo de errores al procesar mensajes
            print(f"Error procesando mensaje: {e}")
            self.enviar_linea(cliente_socket, "ERROR|Error procesando mensaje")
            return None
    ...
    def manejar_conexion(self, nombre, cliente_socket, direccion):
        if not nombre.strip(): # Validar nombre no vacío
            self.enviar_linea(cliente_socket, "ERROR|Nombre vacío")
            return None
        
        if nombre in self.clientes: # Validar nombre único
            self.enviar_linea(cliente_socket, "ERROR|Nombre ya existe")
            return None
        ...
        self.enviar_linea(cliente_socket, "SUCCESS|Conectado exitosamente")
    ...
    def manejar_mensaje_privado(self, parametros, remitente):
        ...
        try: # Procesar mensaje privado
            ...
            if destinatario in self.clientes and self.clientes[destinatario]['conectado']:
                ...
                self.enviar_linea(self.clientes[destinatario]['socket'], mensaje_formateado)
                
                # Confirmar envío al remitente
                self.enviar_linea(self.clientes[remitente]['socket'], "SUCCESS|Mensaje enviado")
                print(f"{remitente} → {destinatario}: {mensaje}")
                
            else:
                # Destinatario no encontrado o desconectado
                self.enviar_linea(self.clientes[remitente]['socket'], "ERROR|Usuario no encontrado o desconectado")
    ...
    def manejar_mensaje_general(self, mensaje, remitente):
        ...
        for nombre, info in self.clientes.items():
            if info['conectado'] and nombre != remitente:  # No reenviar al remitente
                try: # Intentar enviar el mensaje
                    self.enviar_linea(info['socket'], mensaje_formateado)
        ...
        # Confirmar envío
        try:
            self.enviar_linea(self.clientes[remitente]['socket'], "SUCCESS|Mensaje enviado al chat general")
    ...
    def enviar_lista_usuarios(self, cliente_socket):
        ...
        self.enviar_linea(cliente_socket, mensaje)
    ...
    def enviar_historial_general(self, cliente_socket):
        try: # Enviar cada mensaje del historial
            for msg in self.historial_general:
                ...
                self.enviar_linea(cliente_socket, mensaje_formateado)
                time.sleep(0.01)  # Pequeña pausa para evitar saturar
            
            # Señal de fin de historial
            self.enviar_linea(cliente_socket, "HISTORY_END|")
    ...
    def notificar_cambio_usuarios(self):
        ...
        for nombre, info in self.clientes.items(): # Enviar a todos los clientes
            try:
                self.enviar_linea(info['socket'], mensaje)
            except:
                clientes_desconectados.append(nombre)
