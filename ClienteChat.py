...
class ClienteChat:
    ...
    def conectar_servidor(self):
        try:
            ...
            # Enviamos por líneas (\n) para que TCP no "pegue" varios mensajes.
            comando_conexion = f"CONNECT|{self.nombre_usuario}\n"
            self.socket_cliente.sendall(comando_conexion.encode('utf-8'))
            # Leemos UNA línea de respuesta
            buffer = ""
            while "\n" not in buffer:
                chunk = self.socket_cliente.recv(1024)
                if not chunk:
                    return False
    ...
    def recibir_mensajes(self):
        ...
        while self.conectado:
            try:
                ...
                # Procesar mensajes completos por línea
                while "\n" in buffer:
                    linea, buffer = buffer.split("\n", 1)
                    linea = linea.strip("\r")
                    if not linea:
                        continue
                    self.procesar_mensaje_recibido(linea)
    ...
    def solicitar_usuarios(self):
        if self.conectado:
            try:
                self.socket_cliente.sendall("GET_USERS|\n".encode('utf-8'))
            except:
                pass
    ...
    def solicitar_historial(self):
        if self.conectado:
            try:
                self.socket_cliente.sendall("GET_HISTORY|\n".encode('utf-8'))
            except:
                pass
