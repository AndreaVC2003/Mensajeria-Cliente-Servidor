...
## Cómo ejecutar el proyecto (listo para entregar)

### Requisitos
- **Python 3.9+** instalado.
- En Windows, corre desde **CMD / PowerShell** o VS Code.

### 1) Ejecutar el servidor
En la PC que funcionará como servidor (misma red que los clientes):

```bash
python ServidorChat.py
```

Verás algo como:
```
Servidor de chat iniciado en 0.0.0.0:12345
```

> Si tu firewall pregunta, permite el acceso en **Red privada**.

### 2) Ejecutar el cliente (interfaz gráfica)
En cada PC cliente:
...
- IP del servidor: (ej. `192.168.1.100`) o `localhost` si es la misma PC
- Puerto: `12345`

### 3) ¿Cómo sé que ya funciona?
Prueba rápida (mínimo):
1. Abre el servidor.
2. Abre **2 clientes** con nombres distintos.
3. Envía un **BROADCAST** y valida que se vea en el otro.
4. Envía un **MESSAGE** privado y valida que solo lo reciba el destinatario.
5. Entra un tercer cliente y confirma que la lista de usuarios cambie.

---

## Nota técnica (importante para TCP)
Como TCP puede **juntar** varios mensajes en una sola lectura (o **partirlos**), el proyecto usa **delimitación por líneas**:
- Cada mensaje viaja como: `COMANDO|...\n`
- El servidor y el cliente procesan **una instrucción por línea**.
